package cdu.zch.rpc.utils.threadpool;
...
public final class ThreadPoolFactoryUtil {
    ...
    /**
     * 通过threadNamePrefix 区分不同线程池（将相同threadNamePrefix的线程池看作是同意业务场景服务）
    ...
    public static ExecutorService createCustomThreadPoolIfAbsent(CustomThreadPoolConfig customThreadPoolConfig, String threadNamePrefix, Boolean daemon) {
        ExecutorService threadPool = THREAD_POOLS.computeIfAbsent(threadNamePrefix, k -> createCustomThreadPoolIfAbsent(customThreadPoolConfig, threadNamePrefix, daemon));
    ...
    /**
     * 创建ThreadFactory。如果threadNamePrefix不为空则使用自建ThreadFactory，否则使用defaultFactory
     * @param threadNamePrefix 作为创建的的线程名字的前缀
    ...
    public static void printThreadPoolStatus(ThreadPoolExecutor threadPool) {
        ScheduledExecutorService scheduledExecutorService =
                new ScheduledThreadPoolExecutor(
                        1,
                        createThreadFactory("print-thread-pool-status",
                        false)
                );
